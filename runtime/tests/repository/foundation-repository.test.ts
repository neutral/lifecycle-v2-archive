...
const REPOSITORY_SCHEMA_ID = "urn:lifecycle:schema:repository-contract:v15";

test("authority secrets use exact UTF-8 byte bounds without trimming", () => {
  assert.throws(() => validateAuthoritySecret(undefined), /32 to 4096 secret bytes/u);
  assert.throws(() => validateAuthoritySecret("a".repeat(31)), /32 to 4096 secret bytes/u);
  assert.equal(validateAuthoritySecret("a".repeat(32)), "a".repeat(32));
  assert.equal(validateAuthoritySecret("a".repeat(4096)), "a".repeat(4096));
  assert.throws(() => validateAuthoritySecret("a".repeat(4097)), /32 to 4096 secret bytes/u);

  const multibyteMinimum = "é".repeat(16);
  assert.equal(Buffer.byteLength(multibyteMinimum, "utf8"), 32);
  assert.equal(validateAuthoritySecret(multibyteMinimum), multibyteMinimum);

  const withTrailingNewline = `${"a".repeat(32)}\n`;
  assert.equal(validateAuthoritySecret(withTrailingNewline), withTrailingNewline);
  assert.throws(
    () => validateAuthoritySecret(`${"a".repeat(32)}\0`),
...
test("the installed Provider Descriptor exactly retains the published first-adapter value", async () => {
  ...
  assert.equal(descriptor.provider.compatibleVersion, ">=0.151.0 <0.152.0");
  assert.equal(descriptor.digest, "sha256:2e7d6aa152145518c6ce35b561384eb9f0e49dd2736ea019f18d47d5f095fc9e");
  assert.deepEqual(
    validateFoundationSchema("urn:lifecycle:schema:provider-descriptor:v6", descriptor, "installed-provider"),
    [],
  );
...
test("the published fresh repository-v15 fixture parses through the runtime contract owner", async () => {
  const published = JSON.parse(await readFile(
    new URL("../../../../spec-source/examples/repository-v15-fresh-valid/.lifecycle/repository.json", import.meta.url),
    "utf8",
  )) as unknown;
...
test("repository identities and contract collections use exact scalar semantics", async () => {
  ...
  const authority = await createFoundationAuthority(home, "scalar-order-target", SECRET);
...
test("repository contract v15 is self-digested and rejects silent mutation", async () => {
  const { root, home } = await repository();
  const authority = await createFoundationAuthority(home, "target-one", SECRET);
  const binding = commandCheckBinding({
    id: "repository-check",
  ...
  assert.deepEqual(contract.selections.profiles, ["knowledge-set-v1", "knowledge-structural-v1", "repository-v7"]);
  assert.deepEqual(contract.selections, {
    schemas: foundationRepositorySchemaIds(),
    profiles: ["knowledge-set-v1", "knowledge-structural-v1", "repository-v7"],
  ...
  assert.deepEqual(Object.keys(defaultProjectionProfiles()), ["orientation-standard-v1", "execution-standard-v1", "execution-large-v1"]);
  ...
  (unsupportedSelection.selections as Record<string, unknown>).profiles = ["knowledge-set-v1", "knowledge-structural-v1", "unsupported-v1"];
  ...
  assert.match(raw, /"\$schema": "lifecycle.repository.v15"/);
});
...
test("repository initialization creates only Foundation roots and stages exact carriers", async () => {
  ...
  assert.deepEqual((await readdir(join(root, "records"))).sort(), ["assurance", "behavior", "blueprint", "checks"]);
  ...
  assert.equal(validation.profile, "repository-v7");
  assert.equal((await git(root, ["status", "--short"])).stdout, "");
});

test("repository v15 refuses repository-visible Control without interpreting its bytes", async () => {
  const { root, home } = await repository();
  ...
  await git(root, ["commit", "-m", "Initialize repository v15 target"]);
...
test("retired repository discriminators fail closed for repository v15 attachment", async () => {
...
test("Foundation repository contracts reject migration provenance", async () => {
  const { home } = await repository();
  const authority = await createFoundationAuthority(home, "fresh-target", SECRET);
  const contract = createRepositoryContract({
...
test("fresh initialization rejects mixed Foundation contract and predecessor Control before new authority", async () => {
  const { root, home } = await repository();
  const existingAuthority = await createFoundationAuthority(home, "existing-foundation", SECRET);
  const contract = createRepositoryContract({
...
test("fresh initialization never replaces an existing machine authority identity", async () => {
  const { root, home } = await repository();
  const existing = await createFoundationAuthority(home, "authority-collision", SECRET);
...
test("Foundation attachment rejects tracked predecessor Control beside a v11 contract", async () => {
  const { root, home } = await repository();
  const authority = await createFoundationAuthority(home, "mixed-attachment", SECRET);
  const contract = createRepositoryContract({
