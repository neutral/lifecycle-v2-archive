...
function tar(entries: readonly Readonly<{ path: string; bytes: Uint8Array; mode: number }>[]): Uint8Array {
  const chunks: Buffer[] = [];
...
function outputArchive(selected: Fixture): Uint8Array {
  ...
  return tar([
    ...
    { path: entries[1]!.path, bytes, mode: 0o644 },
  ]);
}
...
class FakeDockerCli implements FoundationDockerCliCommandExecutorV1 {
  ...
  async execute(request: Parameters<FoundationDockerCliCommandExecutorV1["execute"]>[0]) {
    ...
    if (arguments_[0] === "network" && arguments_[1] === "ls") {
      const filter = value(arguments_, "--filter") ?? "";
      const match = /^name=\^(.*)\$$/u.exec(filter);
      const name = match?.[1];
      return this.#result(name !== undefined && this.networks.has(name) ? `${name}\n` : "");
    }
    ...
    if (arguments_[0] === "container" && arguments_[1] === "create") {
      ...
      const id = name.endsWith("input-stage")
        ? STAGING_ID
    ...
    if (arguments_[0] === "container" && arguments_[1] === "kill") {
      this.#container(arguments_[2]!)!.state = "exited";
      return this.#result(`${arguments_[2]}\n`);
    }
...
function inputTransport(selected: Fixture): FoundationDockerInputSetTransportResolverV1 {
...
function providerSupport(selected: Fixture): FoundationDockerAgentProviderSupportResolverV1 {
  ...
  return Object.freeze({
    ...
    async openCredential(
      ...
      assert.equal(input.specificationDigest, selected.specification.digest);
